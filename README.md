...
- `TRADING_CORE_WS_CONDITION_EVENT_WORKERS`: shard Core WS `condition_event` batches by stock code across worker queues while preserving per-code order; default `2`, allowed range `1..8`. Status exposes `core_condition_event_worker_count`, `core_condition_event_active_worker_count`, `core_condition_event_last_worker_index`, and `core_condition_event_last_shard_key`.
