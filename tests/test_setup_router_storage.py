...
def test_setup_router_state_ignores_price_only_observation_transition(tmp_path):
    ...
    states = db.list_setup_router_states(trade_date=TRADE_DATE, candidate_instance_ids=["ci-1"], router_version="setup_router_v3.5.2")
...
def test_atomic_completion_rolls_back_partial_writes_on_injected_failure(tmp_path):
    ...
    assert db.list_setup_router_states(trade_date=TRADE_DATE, candidate_instance_ids=["ci-1"], router_version="setup_router_v3.5.2") == []
...
def test_setup_router_state_version_isolation_between_v34_and_v35(tmp_path):
    ...
    current_rows = db.list_setup_router_states(trade_date=TRADE_DATE, candidate_instance_ids=["ci-1"], router_version="setup_router_v3.5.2")
    ...
    new_table_count = db.conn.execute("SELECT COUNT(*) FROM setup_router_state_v3 WHERE trade_date = ? AND router_version = 'setup_router_v3.5.2'", (TRADE_DATE,)).fetchone()[0]

    assert legacy_rows[0]["router_version"] == "setup_router_v3.4"
    assert current_rows[0]["router_version"] == "setup_router_v3.5.2"
