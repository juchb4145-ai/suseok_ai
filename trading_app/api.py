...
def setup_router_v3_summary(trade_date: str | None = Query(None)) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "router_version": run.get("router_version") or "setup_router_v3.5",
...
def setup_router_v3_candidate(code: str, trade_date: str | None = Query(None), limit: int = Query(50, ge=1, le=500)) -> dict[str, Any]:
    ...
    try:
        limit_value = _setup_router_v3_limit(limit)
        observations = db.list_setup_observations_latest(trade_date=resolved, code=code, router_version="setup_router_v3.5", limit=limit_value)
        transitions = db.list_setup_observation_transitions(trade_date=resolved, code=code, router_version="setup_router_v3.5", limit=limit_value)
        return {
            "trade_date": resolved,
            "code": code,
