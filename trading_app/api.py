...
def setup_router_v3_summary(trade_date: str | None = Query(None)) -> dict[str, Any]:
    ...
    try:
        ...
        latest = list(getattr(db, "list_setup_observations_latest", lambda **_: [])(trade_date=resolved, router_version="setup_router_v3.4.1", limit=1000) or [])
        ...
        return {
            "trade_date": resolved,
            "schema_version": run.get("schema_version") or "setup_router_v3.observe.v4.1",
            "feature_schema_version": run.get("feature_schema_version") or "setup_router_v3.features.v4.1",
            "router_version": run.get("router_version") or "setup_router_v3.4.1",
            "state_version": run.get("state_version") or "setup_router_v3.state.v3.1",
...
def setup_router_v3_observations(
    ...
    router_version: str | None = Query("setup_router_v3.4.1"),
    ...
    try:
        ...
        kwargs = {
            ...
            "router_version": _setup_router_v3_text(router_version) or "setup_router_v3.4.1",
...
def setup_router_v3_candidate(code: str, trade_date: str | None = Query(None), limit: int = Query(50, ge=1, le=500)) -> dict[str, Any]:
    ...
    try:
        limit_value = _setup_router_v3_limit(limit)
        observations = db.list_setup_observations_latest(trade_date=resolved, code=code, router_version="setup_router_v3.4.1", limit=limit_value)
        transitions = db.list_setup_observation_transitions(trade_date=resolved, code=code, router_version="setup_router_v3.4.1", limit=limit_value)
        return {
            "trade_date": resolved,
            "code": code,
...
def _setup_router_v3_int_or_none(value: Any) -> int | None:
    if value is None or value.__class__.__name__ == "Query":
        return None
    try:
        return int(value)
...
def _setup_router_v3_bool_or_none(value: Any) -> bool | None:
    if value is None or value.__class__.__name__ == "Query":
        return None
    ...
    if "position_risk" in runtime:
        runtime["position_risk"] = _dashboard_field_subset(
            ...
            (
                ...
                "gross_open_exposure_krw",
                "gross_pending_buy_exposure_krw",
                "gross_reserved_exposure_krw",
                "gross_available_exposure_krw",
                "gross_utilization_pct",
                "composite_market_mode",
                "systemic_risk_off",
                "market_context_calculated_at",
