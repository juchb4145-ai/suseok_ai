...
목표는 Reboot V2 판단 결과를 제한적 `LIVE_SIM` 주문 관리 경로로 연결하되 REAL 주문은 계속 fail-closed로 막는 것이다.

구현 범위:

- `OrderManagerConfig`, `OrderSide`, `OrderIntentSource`, `OrderIntentStatus`, `ManagedOrderStatus` 등 주문 관리 enum/model 추가
- `OrderRiskManager`로 broker env, whitelist, mode flag, order amount/quantity, daily/code/theme/position limit, RISK_OFF, PositionRisk, kill switch 검사
- `OrderManagerRuntimePipeline`으로 EntryEngine BUY intent와 ExitEngine SELL intent를 local SQLite에 먼저 저장한 뒤 risk PASS일 때만 Gateway command 생성
- `UnfilledCancelScheduler`로 ACK/PARTIAL 미체결 주문의 자동 cancel command 생성
- `ManagedOrderReconciler`로 command_ack와 Chejan execution_event reconcile 분리
- `managed_order_intents`, `managed_orders`, `managed_order_events`, `order_risk_decisions`, `order_kill_switch_state` SQLite 테이블 추가
- dashboard/runtime에 `order_manager` section 추가
- 운영 문서 `docs/live_sim_order_manager_runbook.md` 추가

금지 경로:

- REAL broker/account 주문 활성화
- `TRADING_ALLOW_REAL_LIVE_ORDERS` 류의 real 주문 플래그
- hybrid gate, final grade, shadow/promotion, threshold A/B 결과의 OrderManager 입력 사용
- command_ack 단독 fill 처리
- PostgreSQL 의존성
