...
| --- | --- | --- |
| `OPEN_ORDERS` | `opt10075` | `KOA_STUDIO_SCREENSHOT + KIWOOM_SIM_CAPTURE / HOLD` |
| `ACCOUNT_POSITIONS` | `opw00018` | `KOA_STUDIO_SCREENSHOT + KIWOOM_SIM_CAPTURE / HOLD` |
| `ACCOUNT_CASH` | `opw00001` | `KOA_STUDIO_SCREENSHOT + KIWOOM_SIM_CAPTURE / HOLD` |

KOA Studio screenshot으로 `opt10075`, `opw00018`, `opw00001`의 TR 입력 계약과 `opw00001` expanded output field list를 확인했다. 2026-06-19에는 실제 모의서버에서 세 TR의 read-only capture/parser smoke도 실행했다. 다만 자동 `broker_reconcile` service staging/reconcile pipeline이 실행 중 Core에서 `NOT_CONFIGURED`였으므로 아직 PASS가 아니라 HOLD로 취급한다. `opw00018`과 `opw00001`은 KOA sample 기준으로 `비밀번호`를 공백으로 입력하고 `비밀번호입력매체구분=00`을 유지한다.
