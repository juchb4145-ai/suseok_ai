        ...
        *,
        ...
        for key, lease in list(self._leases.items()):
            ...
            if not _before(current, lease.expires_at):
                ...
                churn += 1
                continue
            self._leases[key] = replace(lease, status=ExpansionLeaseStatus.REMOVAL_PENDING.value, removal_pending_at=current.isoformat(), reason_codes=tuple(_dedupe([*lease.reason_codes, "THEME_NO_LONGER_ELIGIBLE"])))
            churn += 1
