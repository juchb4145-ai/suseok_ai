...
param(
    ...
    [switch]$EnableFullSessionAudit,
    [int]$AuditIntervalSec = 15,
    [string]$AuditEndTime = "15:40",
...
function Set-CoreFullSessionAuditEnv {
    # Make Theme Rotation P0/P1/P2-Min settings deterministic for the Core child process.
    $env:TRADING_INTRADAY_THEME_DISCOVERY_ENABLED = "true"
    $env:TRADING_INTRADAY_THEME_DISCOVERY_START = "09:20"
    $env:TRADING_INTRADAY_THEME_DISCOVERY_END = "15:00"
    $env:TRADING_INTRADAY_THEME_DISCOVERY_MORNING_INTERVAL_SEC = "300"
    $env:TRADING_INTRADAY_THEME_DISCOVERY_MIDDAY_INTERVAL_SEC = "600"
    $env:TRADING_INTRADAY_THEME_DISCOVERY_ROTATION_INTERVAL_SEC = "300"
    $env:TRADING_INTRADAY_THEME_DISCOVERY_LATE_INTERVAL_SEC = "600"
    $env:TRADING_INTRADAY_THEME_DISCOVERY_TOP_N = "100"
    $env:TRADING_INTRADAY_THEME_DISCOVERY_SIGNAL_TTL_SEC = "600"
    $env:TRADING_INTRADAY_THEME_DISCOVERY_MAX_PENDING_COMMANDS = "1"
    $env:TRADING_INTRADAY_THEME_DISCOVERY_QUEUE_DEPTH_LIMIT = "5"

    $env:TRADING_THEME_SIGNAL_TTL_SEC = "600"
    $env:TRADING_MARKET_DATA_MAX_TICK_AGE_SEC = "10"
    $env:TRADING_THEME_TURNOVER_FLOW_ENABLED = "true"
    $env:TRADING_THEME_LEADERSHIP_HANDOVER_ENABLED = "true"
    $env:TRADING_THEME_BRIDGE_RECONCILE_ENABLED = "true"

    $env:TRADING_THEME_MIN_LEADING_PERSISTENCE_CYCLES = "3"
    $env:TRADING_THEME_MIN_LEADING_PERSISTENCE_SEC = "30"
    $env:TRADING_THEME_MIN_LEADER_STABILITY_CYCLES = "2"
    $env:TRADING_THEME_MIN_LEADER_STABILITY_SEC = "20"
    $env:TRADING_THEME_STATE_MIN_DWELL_SEC = "15"
    $env:TRADING_THEME_FADING_MIN_HOLD_SEC = "30"
    $env:TRADING_THEME_RECOVERY_CONFIRM_CYCLES = "3"
    $env:TRADING_THEME_RECOVERY_CONFIRM_SEC = "30"
    $env:TRADING_THEME_SIGNAL_STALE_GRACE_SEC = "20"

    $env:TRADING_DIRTY_EVALUATOR_ENABLED = "true"
    $env:TRADING_DIRTY_EVALUATOR_SHADOW_MODE = "true"
    # Avoid a full candidate scan on every dirty event during the all-day qualification.
    $env:TRADING_DIRTY_EVALUATOR_FALLBACK_FULL_SCAN = "false"
    $env:TRADING_DIRTY_EVALUATOR_ORDER_INTENT_ENABLED = "false"
    $env:TRADING_DIRTY_EVALUATOR_THEME_CADENCE_SEC = "1"
    $env:TRADING_DIRTY_EVALUATOR_MARKET_CADENCE_SEC = "1"
}

function Start-FullSessionAuditSampler(
    [string]$Python64,
    [string]$TradeDate,
    [string]$OutputDir,
    [int]$IntervalSec,
    ...
    $auditTool = Join-Path $ProjectRoot "tools\audit_strategy_context_v3_intraday.py"
    if (-not (Test-Path $auditTool)) {
        throw "Full-session audit tool not found: $auditTool"
    }
    $endAt = [datetime]::ParseExact(
        "$TradeDate $EndTime",
        "yyyy-MM-dd HH:mm",
        [System.Globalization.CultureInfo]::InvariantCulture
    )
    $durationSec = [int][Math]::Floor(($endAt - (Get-Date)).TotalSeconds)
    if ($durationSec -lt 600) {
        throw "Audit end time $EndTime is too close or already passed. Start before the market and use an end time after 15:30."
    }
    New-Item -ItemType Directory -Path $OutputDir -Force | Out-Null
    $auditOut = Join-Path $LogDir "theme_rotation_full_session_audit_$Timestamp.out.log"
    $auditErr = Join-Path $LogDir "theme_rotation_full_session_audit_$Timestamp.err.log"
    $auditArgs = @(
        $auditTool,
        "--core-url", $CoreUrl,
        "--token", $Token,
        ...
        "--interval-sec", "$([Math]::Max(5, $IntervalSec))",
...
if ($DryRun) {
    [pscustomobject]@{
        ...
        full_session_audit_enabled = [bool]$EnableFullSessionAudit
        audit_interval_sec = $AuditIntervalSec
...
$auditRun = $null
if ($EnableFullSessionAudit) {
    $Python64 = Join-Path $ProjectRoot "venv_64\Scripts\python.exe"
    if (-not (Test-Path $Python64)) {
        $Python64 = "python"
    }
    $tradeDate = Get-Date -Format "yyyy-MM-dd"
    if (-not $AuditOutputDir) {
        $AuditOutputDir = Join-Path $ProjectRoot ("reports\strategy_context_v3\{0}\full_session_{1}" -f $tradeDate, $Timestamp)
    }
    $auditRun = Start-FullSessionAuditSampler `
        -Python64 $Python64 `
        -TradeDate $tradeDate `
        -OutputDir $AuditOutputDir `
        -IntervalSec $AuditIntervalSec `
        -EndTime $AuditEndTime
    Write-Step "Full-session audit sampler started pid=$($auditRun.process.Id) duration_sec=$($auditRun.duration_sec) output=$($auditRun.output_dir)"
}
...
$summary = [pscustomobject]@{
    ...
    reconcile_tr_include_cash = [bool]$ReconcileTrIncludeCash
    full_session_audit_enabled = [bool]$EnableFullSessionAudit
    audit_pid = if ($auditRun) { $auditRun.process.Id } else { $null }
    audit_duration_sec = if ($auditRun) { $auditRun.duration_sec } else { 0 }
    audit_output_dir = if ($auditRun) { $auditRun.output_dir } else { "" }
    audit_stdout = if ($auditRun) { $auditRun.stdout } else { "" }
    audit_stderr = if ($auditRun) { $auditRun.stderr } else { "" }
    core_stdout = $CoreOut
    core_stderr = $CoreErr
    gateway_stdout = $GatewayOut
    gateway_stderr = $GatewayErr
